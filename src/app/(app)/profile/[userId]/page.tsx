...
import type { User, FounderProfile, InvestorProfile, ExpertProfile, ConnectionStatus, ProfileData } from "@/lib/types";
...
const defaultBannerUrl = "https://i.ibb.co/Fkq0myrx/Screenshot-2025-05-27-155445.png";
...
export default function UserProfilePage({ params: paramsPromise }: { params: { userId: string } }) {
  ...
  const getConnectionStatus = (): ConnectionStatus => {
    if (!loggedInUser || !viewedUser || isOwnProfile) return 'not_connected';
  ...
  const handleConnect = async () => {
    ...
    if (result.success) {
      ...
      // Force re-render or rely on useEffect to refresh viewedUser state
      setViewedUser(mockUsers.find(u => u.id === userId)); 
    } else {
        ...
        <div className="relative px-6">
          <div className="flex flex-col md:flex-row items-start gap-6 -mt-16 md:-mt-20">
            ...
            <div className="pt-16 md:pt-20 flex-1">
              ...
              {(viewedUser.profile as ProfileData).location && <p className="text-sm text-muted-foreground mt-1 flex items-center"><MapPin className="h-4 w-4 mr-2"/>{(viewedUser.profile as ProfileData).location}</p>}
              {(viewedUser.profile as ProfileData).website && <Link href={(viewedUser.profile as ProfileData).website!} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline mt-1 flex items-center"><LinkIcon className="h-4 w-4 mr-2"/>{(viewedUser.profile as ProfileData).website}</Link>}
            </div>
          </div>
        
          <div className="py-6">
            {(viewedUser.profile as ProfileData).bio && (
              <div className="mb-6">
                ...
                <p className="text-muted-foreground whitespace-pre-line">{(viewedUser.profile as ProfileData).bio}</p>
              </div>
            )}
            ...
            {(viewedUser.profile as ProfileData).language && (
                 <p className="text-xs text-muted-foreground mt-1 flex items-center"><Globe className="h-3 w-3 mr-1"/>Language: {(viewedUser.profile as ProfileData).language}</p>
            )}
          </div>
        </div>
      </Card>
