...
interface AuthContextType {
  ...
  loginWithLinkedIn: () => Promise<{ success: boolean; error?: string }>; // Added
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    // Simulate initial auth check (e.g., from localStorage for mock)
    setIsLoading(true);
    ...
    if (storedUserJson) {
      try {
        ...
        if (liveMockUser) {
          ...
          localStorage.removeItem('startupconnect-mock-user'); // Stored user no longer in mock data
        }
      } catch (e) {
  ...
  const updateUserStateAndStorage = (updatedUser: User | null) => {
    ...
    if (updatedUser) {
      ...
      // Also update the mockUsers array if the user is being modified
  ...
  const loginWithGoogle = async (): Promise<{ success: boolean; error?: string }> => {
    setIsLoading(true);
    // Simulate Google login with a default mock user
    let mockGoogleUser = mockUsers.find(u => u.email === 'founder@example.com') || mockUsers[0];
    if (!mockGoogleUser && mockUsers.length > 0) mockGoogleUser = mockUsers[0];
  ...
  const loginWithLinkedIn = async (): Promise<{ success: boolean; error?: string }> => {
    setIsLoading(true);
    // Simulate LinkedIn login - let's use the 'expert' user or the second one if available
    let mockLinkedInUser = mockUsers.find(u => u.email === 'expert@example.com') || (mockUsers.length > 1 ? mockUsers[1] : mockUsers[0]);
    
    if (!mockLinkedInUser) { // Fallback
       mockLinkedInUser = {
        id: generateMockId(),
        email: 'linkedinuser@example.com',
        name: 'LinkedIn User',
        role: UserRole.IndustryExpert,
        profile: { areaOfExpertise: EXPERTISE_AREAS[1], yearsOfExperience: 7 } as ExpertProfile,
        connections: [], connectionRequestsSent: [], connectionRequestsReceived: [],
        createdAt: new Date().toISOString(),
      };
      mockUsers.push(mockLinkedInUser);
    }

    updateUserStateAndStorage(mockLinkedInUser);
    setIsLoading(false);
    return { success: true };
  };
  ...
  const signupWithEmailPassword = async (
    ...
    const newUser: User = {
      ...
      profile: {
        ...
        language: (profileData as BaseProfile).language || 'en',

        ...(role === UserRole.Founder && profileData as Partial<FounderProfile>),
        ...( (role === UserRole.AngelInvestor || role === UserRole.VC) && profileData as Partial<InvestorProfile>),
        ...(role === UserRole.IndustryExpert && profileData as Partial<ExpertProfile>),
      } as ProfileData, // Ensure type assertion
      connections: [], connectionRequestsSent: [], connectionRequestsReceived: [],
      createdAt: new Date().toISOString(),
    };
  ...
  const updateUserInContext = async (updatedUserData: Partial<User>): Promise<{ success: boolean; error?: string }> => {
    ...
    updateUserStateAndStorage(updatedUser); // Update context and localStorage
    return { success: true };
  };
  ...
  const sendConnectionRequest = async (targetUserId: string): Promise<{ success: boolean; error?: string }> => {
    ...
    const currentUserData = { ...mockUsers[currentUserIndex] }; // Create a shallow copy
    ...
    const targetUserData = mockUsers[targetUserIndex]; // This will be updated directly in mockUsers
    targetUserData.connectionRequestsReceived = [...new Set([...(targetUserData.connectionRequestsReceived || []), user.id])];
    
    updateUserStateAndStorage(currentUserData); // This also updates mockUsers[currentUserIndex]
    return { success: true };
  };
  ...
  const joinCommunity = async (communityId: string): Promise<{ success: boolean; error?: string }> => {
    ...
    // Components relying on community data will re-render if their props change or they fetch new data.
    return { success: true };
  };
  ...
  const createMockChat = async (participantIds: string[]): Promise<{ success: boolean; chatId?: string; error?: string, chat?: Chat }> => {
    ...
    let existingChat = mockChats.find(chat => {
        if (sortedParticipantIds.length === 2 && !chat.isGroupChat) { // 1-on-1 chat check
            return chat.participantIds.length === 2 && 
                   chat.participantIds.every(id => sortedParticipantIds.includes(id));
        }
        // Basic group chat check by participants - can be more robust for real apps
